.data
    newline: .asciiz "\n"   # Definición de la etiqueta newline para imprimir un salto de línea
...
main:
    beq $s0, $0, iniz   # Si $s0 es igual a cero, salta a iniz

    ble $s4, $s0, sum  # Compara $s4 con $s0. Si $s4 es menor o igual a $s0, salta a sum
...
sum:
    ...
    add $a3, $s1, $s2  # Realiza la suma de $s1 y $s2 y guarda el resultado en $a3
    ...
    la $a0, newline    # Carga la dirección de la etiqueta newline en $a0 para imprimir un salto de línea
